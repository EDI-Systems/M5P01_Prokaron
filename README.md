...
&ensp;&ensp;&ensp;&ensp;The **absolute minimum** value for RMP is about **1.6k ROM and 432 byte RAM**, which is reached on the HC32L136K8TA (Cortex-M0+) port, and this number even included the 60-byte thread control block and 256-byte stack of the first thread. The OS kernel and the stripped down HAL only consumes **116 bytes** of memory combined. If you are willing to push this limit even further, then **manufacturer HAL is a rip-off** and you can roll your own.  
