...
This software is **triple-licensed**: it is either **[LGPL v3](LICENSE.md)** or **[modified MIT license](MODMIT.md)**. **Commercial** licenses are also available upon request. There are exceptions to the LGPLv3: if the kernel itself is not modified, even if it is statically linked to the application, it is considered as normal use. Adding a new port to the kernel **is** a modification to the kernel, and adding drivers **is not** considered as a modification.
