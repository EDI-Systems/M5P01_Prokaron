 ...
 *  rov_setRoots(s)           - set the ROV session specific absolute
 *                              paths to locate relative urls; s is a ';'
 *                              separated list of absolute paths
 *  rov_getSessionId()        - relative paths starting with ~<sid>/ are
 *                              found along the paths specified by
 *                              XDC_ROV_HTTPROOTS and the paths specified
 *                              by rov_setRoots()
 */
/* global rov_setFileDefaults rov_setRoots rov_getSessionId */
    ...
    /* TI Cloud */
        ...
        .then(
            ...
            function(files) {
                ...
                if (rov_setRoots) {
                    console.log("setRoots for session " + rov_getSessionId());
                    rov_setRoots(obj.xdcpath);
                }
...
function _validateExe(path, files)
{
    ...
    if (files.symbolFiles.length > 0) {
        for (var i = 0; i < files.symbolFiles.length; i++) {
            dpath = files.symbolFiles[i].replace(/\\/g, '/');
            dpaths.push(dpath);
        }

        /* check for miracles: user specified fpath == a debugger path */
        for (i = 0; i < dpaths.length; i++) {
            dpath = dpaths[i];
            if (dpath == fpath)  {
                if (dpaths.length > 1) {
                    dpaths.splice(i, 1);
                    suffix += "&symbols=" + dpaths.join(';');
                    //console.log("user named path:" + dpath + suffix);
                }
                return (dpath + suffix);
            }
        }

        /* no match, so assume first debugger path is the exe */
        dpath = dpaths.shift();
    }

    if (dpaths.length > 0) {
        suffix += "&symbols=" + dpaths.join(';');
        //console.log(dpath + suffix);
    }
    ...
    if (dpath != "" && fpath != "undefined" && fpath != "") {
        msg = "the debugger loaded '" + [dpath].concat(dpaths)
