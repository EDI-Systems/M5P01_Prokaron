...
(function (TICloudAgent) {
    ...
    function launchHostApp(agentConfig) {
        ...
        function initGUIComposer() {
            ...
            // need <install directory>/win32/TICloudAgentHostApp
            ...
            workingDir = pathObj.join(workingDir, "TICloudAgentHostApp");
            ...
            lp.stdout.on("data", function (data) {
                ...
                try {
                    ...
                    // ignore all stdout that is not valid JSON
                }
            });
    ...
    function Init() {
        if (!cachedInit) {
            cachedInit = loadAgentConfig()
                ...
                p = Q.defer();   
                createClientModule("127.0.0.1", initParams.port).then(function (agent) {
                    p.resolve(agent);
                }).catch(function (err) {
                    createClientModule("localhost", initParams.port).then(function (agent) {
                        p.resolve(agent);
                    }).catch(function (err) {
                        p.reject(err);
                    });
